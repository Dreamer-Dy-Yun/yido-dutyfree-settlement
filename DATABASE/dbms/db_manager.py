...
from typing import TypeVar, Any, AsyncContextManager, TypeAlias
import pandas as pd

TableModel = TypeVar('TableModel', bound=DeclarativeBase)
DBBatchResult: TypeAlias = dict[str, int | pd.DataFrame | list[str]]
...
class DBManager(ABC):
    ...
    async def upsert_batch(
        ...
        Returns:
            처리 결과 요약 딕셔너리. 부분 커밋 실패가 있으면 errors에 배치 오류를 담는다.
        """
        ...
