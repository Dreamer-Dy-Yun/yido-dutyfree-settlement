...
export const getUsage = async (startDate = null, endDate = null) => {
  const response = await api.get('/api/tenant/usage', {
...
export const getUserTokenUsage = async (userId, startDate = null, endDate = null) => {
  const response = await api.get(`/api/tenant/usage/users/${userId}/tokens`, {
    params: buildUsageDateParams(startDate, endDate),
  });
  return response.data;
};
 ...
 * @param {Object} [options]
 * @param {'ocr'|'verified'} [options.receiptSource] - 목록 행과 동일한 OCR/검증 행을 지정 (receiptId 랑 함께)
 * @param {number} [options.receiptId]
 * @param {'ocr'|'verified'} [options.passportSource]
 * @param {number} [options.passportId]
 */
