...
from DATABASE import pg_manager, models, cruder
...
class FileRetriever:
    ...
    def __init__(
        ...
        self.db: pg_manager.PGDBManager = db
        ...
        self.cruder: cruder.CRUDer = cruder.CRUDer(db)
    ...
    async def set_status(self, measured_by:str, dir_source_base:Path, dir_desti_base:Path, dir_desti_sub:Path):
        ...
        model_names: list[str] = await self.cruder.get_model_names()
        ...
        target_files: set = await self._get_targetfile_set_from_ict(dir_sources, from_datetime, to_datetime)
        ...
        for fullpath_source in target_files:
            ...
            record: dict ={}
            record.update({md.instrument_name: self.instrument_name})
            record.update({md.path_full_source: fullpath_source})
            # data.update({"file_hash": ""}) 
            record.update({md.path_full_current : fullpath_desti})
            record.update({md.is_retrieved: is_retrieved}) 
            data.append(record)
        await self.db.upsert_dataframe(md, pd.DataFrame(data))
    ...
    async def retrieve_files(self,dir_destination:Path):
        ...
        """
        file_list = await self._get_unretrieved_files()
        ...
        # DB 업서트
        await self.db.upsert_dataframe(models.Process, pd.DataFrame(result_lists))


    async def _get_targetfile_set_from_ict(
        self, 
    ...
    async def _get_unretrieved_files(self) -> list:
        md: type[models.Process] = models.Process
        async with self.db as session:
            result:list = await session.execute(select(md.fullpath_source).where(md.is_retrieved == False))
            self._unretrieved_files = result.scalars().all()
            return self._unretrieved_files
