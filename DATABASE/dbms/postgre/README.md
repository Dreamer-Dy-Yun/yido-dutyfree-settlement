...
| Path | Responsibility |
| --- | --- |
| `pg_manager.py` | Public facade. Exports `PGDBManager` and re-exports `DataBaseMaker`; owns engine/session factory initialization, raw query execution, and the explicit DBManager contract. |
| `pg_components.py` | Composition root for `PGDBManager`; wires schema, batch, converter, constraint, and executor components while preserving legacy facade method access. |
| `pg_database.py` | `DataBaseMaker`: development/test database existence check and database creation. |
| `pg_schema.py` | `PGSchemaManager`: schema search-path, schema lifecycle, table create/drop/copy, and truncate behavior. |
| `pg_batch.py` | `PGBatchWriter`: DataFrame batch `upsert`/`update`, statement generation, and legacy `_upsert_dataframe` wrapper. |
| `pg_batch_executor.py` | `PGBatchExecutor`: shared batch execution loop, partial-commit result assembly, and batch error formatting. |
| `pg_dataframe.py` | `PGDataFrameConverter`: DataFrame value conversion based on SQLAlchemy model column types. |
| `pg_constraints.py` | `PGConstraintInspector`: SQLAlchemy metadata inspection for PK/UK/FK, nullability, and conflict-key selection. |
...
Do not refactor or simplify these modules without explicit user approval. The public surface remains `DATABASE.dbms.postgre.pg_manager.PGDBManager`; the split modules are private responsibility units composed behind that facade.
...
- Batch result is a runtime `dict` with the fixed typed keys `cnt_success_rows`, `cnt_failed_rows`, `df_failed`, and `errors`.
