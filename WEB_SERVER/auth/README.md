...
| Path | Responsibility |
| --- | --- |
| `config.py` | Auth-related environment settings, OAuth2 scheme, JWT secret/algorithm/expiration defaults, and production secret fail-fast checks. |
...
- Production environments (`APP_ENV`, `ENV`, or `ENVIRONMENT` set to `prod`/`production`) must provide a non-default `JWT_SECRET_KEY`.
